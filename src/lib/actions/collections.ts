...
async function getProductIdsByAutomationRules(
    ...
    for (const rule of rules.rules) {
        ...
        if (!validFields.includes(rule.field as any)) {
...
// } from "@/lib/db/schema/collections";
// import { products } from "@/lib/db/schema/products";
// import { eq, asc, and, or, lte, gte, isNull, ilike, desc, sql, inArray } from "drizzle-orm";
// import { revalidatePath } from "next/cache";
...
// // ============================================
// // GET: Single collection by slug (for frontend)
// // ============================================
// export async function getCollectionBySlug(slug: string) {
//     try {
//         const [collection] = await db
...
//         if (collection.collectionType === 'manual') {
//             // Manual: Get products from junction table
...
//                 .orderBy(asc(productCollections.sortOrder));
//         } else {
//             // Automated: Build query from rules
//             collectionProducts = await getProductsByAutomationRules(
//                 collection.automationRules as AutomationRules | null
//             );
//         }
...
// type AutomationRule = {
//     field: keyof typeof products.$inferSelect;
//     operator: 'equals' | 'in' | 'gte' | 'lte' | 'gt' | 'lt';
//     value: string | number | boolean | string[] | number[] | null;
// };
...
// // Replace the getProductsByAutomationRules function with this:
// async function getProductsByAutomationRules(rules: AutomationRules | null) {
//     if (!rules || !rules.rules || rules.rules.length === 0) {
//         return [];
//     }

//     const conditions = rules.rules.map((rule) => {
//         const field = products[rule.field];
...
//         switch (rule.operator) {
//             case 'equals':
//                 return eq(field, rule.value);
//             case 'in':
//                 return Array.isArray(rule.value)
//                     ? inArray(field, rule.value as any[])
//                     : null;
//             case 'gte':
//                 return gte(field, rule.value);
//             case 'lte':
//                 return lte(field, rule.value);
//             case 'gt':
//                 return sql`${field} > ${rule.value}`;
//             case 'lt':
//                 return sql`${field} < ${rule.value}`;
//             default:
//                 return null;
...
//     const whereClause = rules.conditions === 'AND'
//         ? and(eq(products.isPublished, true), and(...conditions))
//         : and(eq(products.isPublished, true), or(...conditions));
