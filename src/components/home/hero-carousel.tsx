...
interface HeroSlide {
  type: 'image' | 'video';
  src: string;
  linkUrl?: string | null; // NEW: Optional link destination
  altText?: string; // NEW: For accessibility
}
...
// Slide Content Wrapper Component - handles clickable vs non-clickable
const SlideContentWrapper: React.FC<{
  linkUrl?: string | null;
  children: React.ReactNode;
  className?: string;
}> = ({ linkUrl, children, className }) => {
  if (linkUrl) {
    return (
      <Link href={linkUrl} className={`block w-full h-full ${className || ''}`}>
        {children}
      </Link>
    );
  }
...
const HeroCarousel: React.FC<HeroCarouselProps> = ({ slides, className = '' }) => {
  ...
  // Play first video on mount if it's a video slide
  useEffect(() => {
    const firstSlide = slides[0];
    const firstVideo = videoRefs.current[0];
    
    if (firstSlide?.type === 'video' && firstVideo) {
      firstVideo.play().catch(() => {
        console.log('Autoplay prevented on first video');
      });
    }
  }, []); // Run once on mount
        ...
        <CarouselContent className="h-full bg-black">
          {slides.map((slide, index) => (
            <CarouselItem key={index} className="hero-slide">
              {/* UPDATED: Wrap slide content with clickable link if linkUrl exists */}
              <SlideContentWrapper linkUrl={slide.linkUrl}>
                <div className="hero-slide-content">
                  {slide.type === 'image' ? (
                    <div 
                      ...
                      role="img"
                      aria-label={slide.altText || 'Hero banner'}
                    />
                  ) : (
                    <video
                      ...
                      aria-label={slide.altText || 'Hero video'}
                    />
                  )}
                  <div className="absolute inset-0 bg-black/10" />
                  
                  {/* NEW: Clickable overlay indicator */}
                  {slide.linkUrl && (
